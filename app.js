...
const firebaseConfig = {
  apiKey: "AIzaSyDtSVFHWF2ZRh0My40g1A_X3HpThZIAYUQ",
  authDomain: "snapledger-finance.firebaseapp.com",
  projectId: "snapledger-finance",
  storageBucket: "snapledger-finance.firebasestorage.app",
  messagingSenderId: "828097578623",
  appId: "1:828097578623:web:9e0ca51f4e4618cdbda14a",
  measurementId: "G-9BNH123V0N" // It's okay to leave this in even without Analytics
};
...
class SnapLedger {
  constructor() {
    ...
    this.categories = ['Groceries', 'Restaurant', 'Home', 'Gas', 'Entertainment', 'Other'];
    this.savedExpenses = [];
    this.user = null;
  ...
  async init() {
    onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        document.getElementById('user-greeting').innerText = `Hello, ${user.displayName}`;
    ...
    // Auth
    this.$('#btn-login')?.addEventListener('click', () => this.login());
    this.$('#btn-logout')?.addEventListener('click', () => this.logout());
    ...
    handleIds.forEach(id => {
      const el = this.$(`#${id}`);
      if (!el) return;
      el.value = localStorage.getItem(`snapledger_${id}`) || '';
      el.addEventListener('input', () => {
        localStorage.setItem(`snapledger_${id}`, el.value.trim());
        this.renderSummaryTotals();
      });
    });
    ...
    this.$('#btn-route-personal').addEventListener('click', () => {
      if (!this.user) {
        this.toast('Please log in to save expenses', 'warning');
        return;
      }
      this.vibrate(10);
      this.populatePersonalScreen();
      this.showScreen('personal');
    });

    this.$('#btn-personal-back')?.addEventListener('click', () => {
      this.vibrate(10);
      this.showScreen('route');
    });

    this.$('#btn-save-personal')?.addEventListener('click', async () => {
      await this.savePersonalExpense();
    });

    this.$('#btn-personal-add-item')?.addEventListener('click', () => {
      this.items.push({ id: this.uid(), name: 'New Item', price: 0 });
      this.renderPersonalItems();
      this.recalculatePersonalTotal();
      // Focus the new name input
      const rows = this.$('#personal-items-list').querySelectorAll('.personal-item-row');
      if (rows.length) rows[rows.length - 1].querySelector('.personal-item-name')?.focus();
    });

    this.$('#personal-tax')?.addEventListener('input', () => {
      this.receiptTax = parseFloat(this.$('#personal-tax').value) || 0;
      this.recalculatePersonalTotal();
    });

    // Dashboard Actions
    this.$('#btn-view-dashboard')?.addEventListener('click', () => {
      if (!this.user) {
        this.toast('Please log in to view your dashboard', 'warning');
        return;
      }
      this.vibrate(10);
    ...
    this.$('#btn-dashboard-back')?.addEventListener('click', () => {
      this.vibrate(10);
      this.resetApp();
    });

    this.$('#dashboard-month')?.addEventListener('change', (e) => {
  ...
  parseReceipt(text) {
    ...
    // 1. Extract merchant (first or second non-empty line)
    let merchant = '';
    if (lines.length > 0) {
      merchant = lines[0].trim();
      if (lines.length > 1 && (merchant.length < 3 || /^\d/.test(merchant) || /\b(date|time|tel|phone|store|receipt|welcome|thank)\b/i.test(merchant))) {
        merchant = lines[1].trim();
      }
    }
    this.merchant = merchant.replace(/^[#*\-.\s]+/, '').replace(/[._*\-]+$/, '').trim() || 'Unknown Merchant';

    // 2. Extract receiptDate
    let dateStr = '';
    const dateRegex = /\b(\d{1,2})[/\-](\d{1,2})[/\-](\d{2,4})\b/;
    const isoDateRegex = /\b(\d{4})[/\-](\d{2})[/\-](\d{2})\b/;
    let dateMatch = text.match(dateRegex);
    if (dateMatch) {
      let mm = dateMatch[1].padStart(2, '0');
      let dd = dateMatch[2].padStart(2, '0');
      let yy = dateMatch[3];
      if (yy.length === 2) yy = '20' + yy;
      dateStr = `${yy}-${mm}-${dd}`;
    } else {
      let isoMatch = text.match(isoDateRegex);
      if (isoMatch) {
    ...
    // 3. Extract receiptTotal
    let highestTotal = 0;
    const totalRe = /\b(total|amount|due|balance|grand\s*total)\b/i;
    ...
    this.receiptTotal = highestTotal;
    ...
    if (items.length > 2) {
      ...
      if (filtered.length >= items.length - 1) {
        finalItems = filtered;
      }
    }

    // Final tax and total assignment
    this.receiptTax = detectedTax;
    if (this.receiptTotal === 0 && finalItems.length > 0) {
      this.receiptTotal = finalItems.reduce((sum, item) => sum + item.price, 0) + this.receiptTax;
    }

    return { items: finalItems, tax: detectedTax };
  }
  ...
  saveGroupToStorage() {
    ...
    const saved = JSON.parse(localStorage.getItem('snapledger_names') || '[]');
    for (const p of this.people) {
      if (!saved.includes(p.name)) saved.push(p.name);
    }
    localStorage.setItem('snapledger_names', JSON.stringify(saved.slice(-20)));
  }
  ...
  buildPayLinks(amount, personName) {
    ...
    const note = encodeURIComponent(`SnapLedger split - ${personName}`);
    let links = '';
      ...
      if (navigator.share) {
        try {
          await navigator.share({
            title: `Pay ${this.formatMoney(bd.total)} — SnapLedger`,
  ...
  renderPaymentRequestScreen(data) {
    ...
    const note = encodeURIComponent(`SnapLedger - ${data.n}`);
  ...
  async shareSplit() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'SnapLedger — Bill Split', text, url: shareData.url });
  ...
  buildSummaryText() {
    const lines = ['🧾 SnapLedger — Bill Split\n'];
  ...
  populatePersonalScreen() {
    ...
    this.$('#personal-total').value = this.receiptTotal.toFixed(2);

    // Populate categories select
    const categorySelect = this.$('#personal-category');
  ...
  renderPersonalItems() {
    const itemsList = this.$('#personal-items-list');
    if (!itemsList) return;

    if (!this.items.length) {
      itemsList.innerHTML = '<div style="font-size: 13px; color: var(--text-muted); text-align: center; padding: 10px 0;">No items yet. Tap \'+ Add Item\' below.</div>';
      return;
    }

    itemsList.innerHTML = this.items.map((item, index) => `
      <div class="personal-item-row" data-index="${index}">
        <input class="input personal-item-name" type="text" value="${this.escapeHtml(item.name)}" data-index="${index}" aria-label="Item name" placeholder="Item name">
        <input class="input personal-item-price" type="number" step="0.01" min="0" value="${item.price.toFixed(2)}" data-index="${index}" aria-label="Item price">
        <button class="personal-item-remove" data-index="${index}" aria-label="Remove item">✕</button>
      </div>
    `).join('');

    // Bind name inputs
    itemsList.querySelectorAll('.personal-item-name').forEach(el => {
      el.addEventListener('input', e => {
        const idx = parseInt(e.target.dataset.index);
        if (this.items[idx]) this.items[idx].name = e.target.value;
      });
    });

    // Bind price inputs
    itemsList.querySelectorAll('.personal-item-price').forEach(el => {
      el.addEventListener('input', e => {
        const idx = parseInt(e.target.dataset.index);
        if (this.items[idx]) {
          this.items[idx].price = parseFloat(e.target.value) || 0;
          this.recalculatePersonalTotal();
        }
      });
    });
    ...
    // Bind remove buttons
    itemsList.querySelectorAll('.personal-item-remove').forEach(el => {
  ...
  recalculatePersonalTotal() {
    const itemsSum = this.items.reduce((sum, item) => sum + (parseFloat(item.price) || 0), 0);
    const tax = parseFloat(this.$('#personal-tax')?.value) || 0;
    const newTotal = itemsSum + tax;
    this.receiptTotal = newTotal;
    const totalInput = this.$('#personal-total');
  ...
  async fetchExpenses() {
    if (!this.user) return;
    try {
      const q = query(collection(db, 'expenses'), where('userId', '==', this.user.uid));
      const snapshot = await getDocs(q);
      this.savedExpenses = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
    } catch (err) {
  ...
  async savePersonalExpense() {
    if (!this.user) {
      this.toast('Please log in to save expenses', 'warning');
      return;
    }

    const saveBtn = this.$('#btn-save-personal');
    if (saveBtn?.disabled) return;

    const merchant = this.$('#personal-merchant').value.trim() || 'Unknown Merchant';
    const date = this.$('#personal-date').value || this.receiptDate;
    const tax = parseFloat(this.$('#personal-tax').value) || 0;
    const total = parseFloat(this.$('#personal-total').value) || 0;
    const category = this.$('#personal-category').value || 'Other';
    ...
    try {
      ...
      this.toast('Could not save expense', 'error');
    } finally {
  ...
  renderDashboard() {
    ...
    this.savedExpenses.forEach(exp => {
      if (exp.date) {
        monthsSet.add(exp.date.substring(0, 7)); // YYYY-MM
      }
    });

    const uniqueMonths = Array.from(monthsSet).filter(Boolean);
    uniqueMonths.sort((a, b) => b.localeCompare(a)); // Newest first

    if (!uniqueMonths.length) {
      this.$('#dashboard-month').innerHTML = '<option value="">No data</option>';
      this.$('#dashboard-total').textContent = '$0.00';
      this.$('#dashboard-chart-card').classList.add('hidden');
      this.$('#dashboard-list').innerHTML = `
        <div class="empty-state">
          <div class="empty-icon">📊</div>
          <p>No saved expenses yet. Scan or add receipts to view insights!</p>
        </div>
      `;
      if (this.categoryChart) {
        this.categoryChart.destroy();
    ...
    this.$('#dashboard-chart-card').classList.remove('hidden');

    const monthSelect = this.$('#dashboard-month');
    monthSelect.innerHTML = uniqueMonths.map(m => 
      `<option value="${m}">${m}</option>`
    ).join('');

    this.updateDashboardView(uniqueMonths[0]);
  }

  updateDashboardView(monthString) {
    if (!monthString) return;

    const filtered = this.savedExpenses.filter(exp => exp.date && exp.date.startsWith(monthString));

    // 1. Calculate Total Spent
    const totalSpent = filtered.reduce((sum, exp) => sum + (parseFloat(exp.total) || 0), 0);
    this.$('#dashboard-total').textContent = this.formatMoney(totalSpent);

    // 2. Render Chronological List
    const sorted = [...filtered].sort((a, b) => b.date.localeCompare(a.date) || b.timestamp - a.timestamp);
    const listContainer = this.$('#dashboard-list');
    
    listContainer.innerHTML = sorted.map(exp => `
      <div class="transaction-card card" data-id="${exp.id}">
        <div class="transaction-info">
          <div class="transaction-merchant">${this.escapeHtml(exp.merchant)}</div>
          <div class="transaction-date">${exp.date}</div>
        </div>
        <div class="transaction-meta">
          <div class="transaction-total">${this.formatMoney(exp.total)}</div>
          <span class="category-badge cat-${this.escapeHtml(exp.category.toLowerCase())}">${this.escapeHtml(exp.category)}</span>
        </div>
        <button class="btn-delete-expense" data-id="${exp.id}" aria-label="Delete expense">🗑️</button>
      </div>
    `).join('');

    // Bind delete buttons
    listContainer.querySelectorAll('.btn-delete-expense').forEach(btn => {
      btn.addEventListener('click', async e => {
        e.stopPropagation();
        this.vibrate(15);
        await this.deleteExpense(btn.dataset.id);
      });
    });

    // 3. Render Chart
    const categorySums = {};
    this.categories.forEach(c => categorySums[c] = 0);
    filtered.forEach(exp => {
      const cat = exp.category || 'Other';
      if (categorySums[cat] !== undefined) {
        categorySums[cat] += parseFloat(exp.total) || 0;
      } else {
        categorySums[cat] = parseFloat(exp.total) || 0;
      }
    });
  ...
  async deleteExpense(id) {
    if (!this.user) {
      this.toast('Please log in to delete expenses', 'warning');
      return;
    }
    const currentMonth = this.$('#dashboard-month')?.value;
    try {
      await deleteDoc(doc(db, 'expenses', id));
      await this.fetchExpenses();
      this.renderDashboard();
      if (currentMonth && this.savedExpenses.some(e => e.date && e.date.startsWith(currentMonth))) {
        const sel = this.$('#dashboard-month');
        if (sel) sel.value = currentMonth;
        this.updateDashboardView(currentMonth);
      }
      this.toast('Expense deleted', 'default');
    } catch (err) {
      console.error('Delete expense failed:', err);
      this.toast('Could not delete expense', 'error');
    }
  }
